          ...
          <div className="hidden md:flex items-center justify-end flex-1">
            {user ? (
              <div className="flex items-center gap-2 md:gap-4">
                {user.role === 'buyer' && <Link to='/profile/wishlist'>
                  <Heart className="cursor-pointer" strokeWidth={1.5} size={20} />
                </Link>}
                {user.role === 'buyer' && <Link to="/cart">
                  <ShoppingCart className="cursor-pointer" strokeWidth={1.5} size={20} />
                </Link>}
                <Link to={user.role === 'buyer' ? "/profile" : user.role === 'seller' ? '/seller-dashboard' : 'admin-dashboard'} >
                  {user?.avatar ? (
                    <img
                      src={user.avatar}
                      alt="profile"
        ...
        <div className={`fixed inset-0 ${isOpen ? "pointer-events-auto" : "pointer-events-none"}`}>
          ...
          <div className={`absolute top-0 left-0 h-full w-72 bg-white p-4 z-10 pointer-events-auto transform transition-transform duration-300 ease-in-out ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
            <div className="flex justify-between items-center">
              {user &&
                <Link to={user.role === 'buyer' ? "/profile" : user.role === 'seller' ? '/seller-dashboard' : 'admin-dashboard'} onClick={() => setIsOpen(false)}>
                  {user?.avatar ?
                    (<img
                      src={user.avatar}
                      alt="profile"
            ...
            {/* 📂 Categories */}
            <div className={` ${user ? "" : "border-t"} text-black flex flex-col`}>
