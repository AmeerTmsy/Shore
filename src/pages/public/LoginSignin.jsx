...
export default function Login() {
  ...
  const validateForm = () => {
    ...
    if (isLogin) {
      ...
      if (!name.trim() || !email.trim() || !password.trim() || !confirmePassword.trim() || !userType) {
  ...
  const handleLogin = async () => {
    ...
    if (isLogin) {
      ...
      const res = await authAPI.signin({ name, email, password, role: userType });
      if (res.success) {
  ...
  return (
    <div className="w-full max-w-6xl flex flex-col mx-auto grow py-10">
